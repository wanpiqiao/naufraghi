...
def train_deep_network(origX, layers, finetune=None, targets=None):
    if finetune == 'Backprop' and not targets:
        raise ValueError("target must be != None with Backprop")
    ...
    if finetune == 'WakeSleep':
        ...
        for i in range(num_layers):
            network[2 * num_layers + 1 - i] = network[i]
            network[i].W = network[i].W.T
            network[i].bias = network[i].bias_upW.T
            network[2 * num_layers + 1 - i].bias = network[i].bias_downW.T
        network[num_layers + 1].type = 'sigmoid'
